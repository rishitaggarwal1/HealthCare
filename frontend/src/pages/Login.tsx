...
export default function Login() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const r = await api.post("/api/auth/login", { email, password, deviceId, deviceName });

      setAuth(r.data.accessToken, r.data.role, r.data.refreshToken, r.data.sessionId);
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 p-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow p-6">
        ...
        <div className="mt-5 space-y-3">
          ...
          <a className="text-sm text-blue-700" href="/register">Create an account</a>
        </div>
      </div>
    </div>
  );
